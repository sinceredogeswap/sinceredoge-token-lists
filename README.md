# SincereDogeSwap Token Lists

This repo contains main SincereDogeSwap token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to SincereDogeSwap default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to SincereDogeSwap (extended) token list
...
- Add new tokens to `src/tokens/sinceredogeswap-extended.json` file
...
# Build token list (SincereDogeswap-top-100.json)
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.sinceredogeswap.finance/list-name.json`
